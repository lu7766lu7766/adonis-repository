...
export declare class Repository<T extends LucidModel> {
    ...
    firstOrCreate(searcPayloadh: Partial<ModelAttributes<InstanceType<T>>>, savePayload?: Partial<ModelAttributes<InstanceType<T>>> | undefined, options?: ModelAssignOptions | undefined): Promise<InstanceType<T>>;
     ...
     * */
    updateOrCreate(search: Partial<ModelAttributes<InstanceType<T>>>, bodies: Partial<ModelAttributes<InstanceType<T>>>, options?: ModelAssignOptions): Promise<InstanceType<T>>;
    updateOrCreateManyByKey(key: keyof ModelAttributes<InstanceType<T>>, bodies: Partial<ModelAttributes<InstanceType<T>>>[], options?: ModelAssignOptions): Promise<[InstanceType<T>[], InstanceType<T>[]]>;
    private trx?;
    ...
    deleteBy(where: {
        [m: string]: string | number | Array<string | number>;
    }): Promise<number[]>;
}
export {};
